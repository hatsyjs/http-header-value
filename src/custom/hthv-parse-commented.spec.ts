...
describe('HthvParser', () => {
  describe('parsing comments', () => {
    it('recognizes empty comment', () => {
      expect(hthvParseCommented('()')).toEqual(items(
          { $: 'raw', v: '' },
      ));
    });
    it('recognizes raw comment', () => {
      expect(hthvParseCommented('(comment)')).toEqual(items(
          { $: 'raw', v: 'comment' },
      ));
    });
    ...
    it('recognizes parameter comment', () => {
      expect(hthvParseCommented('(comment:param)')).toEqual(items(
    ...
    it('recognizes tagged comment', () => {
      expect(hthvParseCommented('(tag"comment")')).toEqual(items(
    ...
    it('unescapes arbitrary symbol', () => {
      expect(hthvParseCommented('(com\\ment)')).toEqual(items(
          { $: 'raw', v: 'comment' },
      ));
    });
    ...
    it('accepts spaces an colons', () => {
      expect(hthvParseCommented('(some comment, for test)')).toEqual(items(
    ...
    it('recognizes comment parameters', () => {
      ...
      expect(hthvParseCommented('(comment; p1:value, with space and colon; p2:"value in quotes")')).toEqual(items(
    ...
    it('recognizes headless comment parameter', () => {
      ...
      expect(hthvParseCommented('(;test param)')).toEqual(items(
    ...
    it('recognizes User-Agent string', () => {
      ...
      expect(hthvParseCommented('Mozilla/5.0 (X11; Linux x86_64; rv:70.0) Gecko/20100101 Firefox/70.0')).toEqual(items(
          { $: 'raw', v: 'Mozilla/5.0' },
