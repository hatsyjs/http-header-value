...
describe('hthvParse', () => {
  ...
  it('recognizes quoted string with suffix', () => {
    expect(hthvParse('"value"suffix')).toEqual(
      items({
        $: 'quoted-string',
        v: 'value',
  ...
  it('recognizes tagged string with suffix', () => {
    expect(hthvParse('prefix"value"suffix')).toEqual(
      items({
        $: 'tagged-string',
        t: 'prefix',
        v: 'value',
  ...
  it('recognizes multi-quote string', () => {
    expect(hthvParse('foo"1""2"bar"3"')).toEqual(
      items({
        $: 'tagged-string',
        t: 'foo',
        v: '1',
  ...
  it('recognizes angle-bracketed-string', () => {
    expect(hthvParse('<string content>')).toEqual(
      items({ $: 'angle-bracketed-string', v: 'string content' }),
    );
  });
  it('recognizes empty angle-bracketed-string', () => {
    expect(hthvParse('<>')).toEqual(items({ $: 'angle-bracketed-string', v: '' }));
  });
  it('recognizes incomplete angle-bracketed-string', () => {
    expect(hthvParse('<incom...')).toEqual(items({ $: 'angle-bracketed-string', v: 'incom...' }));
  });
  ...
  it('allows angle bracket after token', () => {
    expect(hthvParse('a<b')).toEqual(items({ $: 'raw', v: 'a<b' }));
  });
  it('allows angle bracket after as part of value', () => {
    expect(hthvParse('a*b<c')).toEqual(items({ $: 'raw', v: 'a*b<c' }));
  });
  ...
  it('recognizes quoted parameter value with prefix and suffix', () => {
    const param = paramItem({
      ...
      n: 'param',
      v: 'prefix',
      x: extras({ $: 'quoted-string', v: 'val' }, { $: 'raw', v: 'suffix' }),
    });

    expect(hthvParse('value;param=prefix"val"suffix')).toEqual(
      items({ $: 'raw', v: 'value', p: { param }, pl: [param] }),
    );
  });
  it('recognizes multi-quote parameter value', () => {
    ...
    expect(hthvParse('value;param="1""2"bar"3"')).toEqual(
      items({ $: 'raw', v: 'value', p: { param }, pl: [param] }),
    );
  });
  it('recognizes quoted parameter with suffix', () => {
    ...
    expect(hthvParse('value;"val"suffix')).toEqual(
      items({ $: 'raw', v: 'value', p: { val: param }, pl: [param] }),
    );
  });
  it('recognizes tagged parameter with suffix', () => {
    ...
    expect(hthvParse('value;prefix"val"suffix')).toEqual(
      items({ $: 'raw', v: 'value', p: { prefix: param }, pl: [param] }),
    );
  });
  it('recognizes multi-quote parameter', () => {
    ...
    expect(hthvParse('value;foo"1""2"bar"3"')).toEqual(
      items({ $: 'raw', v: 'value', p: { foo: param }, pl: [param] }),
    );
  });
  it('does not recognize date/time parameter value', () => {
    ...
    expect(hthvParse(`id=fg12dhd; Expires=${timestamp}`)).not.toEqual(
      items({ $: 'raw', n: 'id', v: 'fg12dhd', p: { Expires: param }, pl: [param] }),
    );
  });
  it('recognizes multiple parameters', () => {
    ...
    const p2 = paramItem({
      $: 'raw',
      v: 'tag',
      x: extras({ $: 'quoted-string', v: 'param' }),
    });
    const p3 = paramItem({
      $: 'raw',
      n: 'bar',
    ...
    expect(hthvParse('some;foo;tag"param";bar=baz')).toEqual(
      items({ $: 'raw', v: 'some', p: { foo: p1, tag: p2, bar: p3 }, pl: [p1, p2, p3] }),
    );
  });
  ...
  it('prefers first parameter value', () => {
    ...
    expect(hthvParse('some;param=1;param=2')).toEqual(
      items({ $: 'raw', v: 'some', p: { param: param1 }, pl: [param1, param2] }),
    );
  });
  it('prefers named parameter over unnamed one', () => {
    ...
    expect(hthvParse('some;param;param=2')).toEqual(
      items({ $: 'raw', v: 'some', p: { param: param2 }, pl: [param1, param2] }),
    );
  });
  it('recognizes comma-separated values', () => {
    expect(hthvParse('foo, bar')).toEqual(items({ $: 'raw', v: 'foo' }, { $: 'raw', v: 'bar' }));
  });
  ...
  it('recognizes parameterized values', () => {
    ...
    expect(hthvParse('foo; p1=v1, bar; p2 baz; p3')).toEqual(
      items(
  ...
  it('treats comments as values', () => {
    expect(hthvParse('(comment1), (comment2)')).toEqual(
      items({ $: 'raw', v: '(comment1)' }, { $: 'raw', v: '(comment2)' }),
    );
  });
});
...
describe('hthvParseDT', () => {
  ...
  it('recognizes date/time parameter value', () => {
    ...
    expect(hthvParseDT(`id=fg12dhd; Expires=${timestamp}`)).toEqual(
      items({ $: 'raw', n: 'id', v: 'fg12dhd', p: { Expires: param }, pl: [param] }),
    );
  });
});
